...
let appState = {
  ...
  hotNumbers: ["--", "--", "--"],
  chartHistory: [
    { id: "1", date: "04-08-2026", result: "113-46" }
  ],
  lastKnownDate: ""
};
...
// SECURITY FIX: Only allow public access to safe files, block backend code and admin HTML
app.use(express.static(path.join(__dirname), {
  setHeaders: (res, filepath) => {
    if (filepath.endsWith('server.js') || filepath.endsWith('package.json') || filepath.endsWith('admin.html')) {
      res.status(403).end('403 Forbidden: Access Denied');
    }
  }
}));

// Root route - serve code.html
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'code.html'));
});

// Legal route - serve legal.html
...
io.on('connection', (socket) => {
  ...
  socket.on('update_site_data', (payload) => {
    if (payload.type === 'field') {
      ...
      if (appState[payload.parent]) {
        appState[payload.parent][payload.field] = payload.value;
      }
...
// AUTOMATED LIVE STATUS & MIDNIGHT RESET
